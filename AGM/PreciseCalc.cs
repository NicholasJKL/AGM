...
namespace AGM
{
	public static class PreciseCalc
	{
		...
		public static double SympsonWaveEq(PreciseFunc f, double r, double t, double R)
		{
			...
			double h = r / 6,
				term1 = (4 * f(r / 2) * (r / 2)) / AGM(Math.Sqrt(Math.Pow(t, 2) - Math.Pow(R - (r / 2), 2)), Math.Sqrt(Math.Pow(t, 2) - Math.Pow(R + (r / 2), 2)), MainWindow.Precision),
				term2 = (f(r) * r) / AGM(Math.Sqrt(Math.Pow(t, 2) - Math.Pow(R - r, 2)), Math.Sqrt(Math.Pow(t, 2) - Math.Pow(R + r, 2)), MainWindow.Precision);
